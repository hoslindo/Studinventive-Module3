...
function traiterClickSurEntrer(event) {
    ...
    // ou sortie du site par un lien "Quitter"
        ...
        window.sessionStorage.setItem("identiteDuCandidat",prenom); // sauvegarde du prénom
...
async function chargementDesDonnees(prenom) { // async requis puisque la fonction fait appel à une fonction avec await
    ...
    window.sessionStorage.setItem("reponses",JSON.stringify(reponses));
    ...
    DEBUG("test suivi")
    if (JSON.parse(window.sessionStorage.getItem("suivi"+prenom))===null) { DEBUG("création suivi")
        ...
        suivi=JSON.parse(window.sessionStorage.getItem("suivi"+prenom));
    }
